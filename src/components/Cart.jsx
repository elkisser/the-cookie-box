...
const Cart = ({ isOpen, onClose }) => {
  ...
  const handleSendInstagram = () => {
    try {
      const plain = `¡Hola! Quiero hacer un pedido de The Cookie Box:\n\n${items.map(item => `• ${item.name} x${item.quantity}`).join('\n')}`;
  ...
  return (
    <>
      {/* Modal - FUERA del contenedor del Cart para evitar problemas de z-index */}
      {showModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black bg-opacity-70 animate-fade-in">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full mx-auto transform animate-scale-in border border-gray-200">
            <div className="p-8 text-center">
              ...
              {/* Título */}
              <h3 className="font-poppins font-black text-3xl text-black mb-4 tracking-tight">
                ¡Pedido Listo!
              </h3>
              
              {/* Descripción */}
              <div className="space-y-3 mb-8">
                <p className="font-poppins text-gray-700 text-lg leading-relaxed font-medium">
                  Mensaje copiado al portapapeles
                </p>
                <p className="font-poppins text-gray-600 text-base leading-relaxed">
                  Pégalo en Instagram para completar tu pedido
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <button
                  ...
                  className="flex-1 font-poppins font-bold text-gray-700 bg-transparent border-2 border-gray-300 hover:border-black hover:text-black transition-all duration-300 py-4 px-6 rounded-xl"
                >
                  Cerrar
                </button>
                <button
                  ...
                  className="flex-1 font-poppins font-bold text-white bg-black hover:bg-gray-900 transition-all duration-300 py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95"
                >
                  Abrir Instagram
                </button>
              </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-black p-6 text-white shadow-lg">
                  ...
                  <div className="space-y-3">                  
                    ...
                    >
                      ...
                      <span>Escribir en Instagram</span>
                    </button>
                  </div>
                </div>
              </>
